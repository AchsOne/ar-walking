    ...
    AndroidView(
        ...
        factory = { ctx ->
            ...
            arSceneView.scene.addOnUpdateListener { _ ->
                ...
                val matchesNow = try { routeViewModel.currentMatches.value } catch (_: Exception) { emptyList<RouteViewModel.LandmarkMatch>() }
                val top = matchesNow.firstOrNull()
                val recognizedNow = top?.let { it.matchCount >= 2 && it.confidence >= 0.15f } == true
                val topInfo = top?.let { "${it.landmark.id} ${it.matchCount}m ${(it.confidence * 100).toInt()}%" } ?: "none"
                ...
                // 2) Enabled required; if not recognized, keep latched arrow if any, otherwise show nothing
                if (!isEnabled) {
                    Log.d("ARCoreArrow", "Arrow hidden: disabled")
                    controller.clear()
                    return@addOnUpdateListener
                }
                if (!recognizedNow) {
                    if (controller.isAnchored()) {
                        ...
                        // No updates; keep current anchor and yaw
                    } else {
                        Log.d("ARCoreArrow", "No recognition and no anchor; arrow hidden")
                        controller.clear()
                    }
                    return@addOnUpdateListener
                }
                ...
                // We have a hit (center or neighbor)
                // Latch landmark and yaw from the recognized instruction
                topMatch?.let { tm ->
                    val newLandmarkId = tm.landmark.id
                    val newYaw = directionDeg
                    if (latchedLandmarkId != newLandmarkId) {
                        latchedLandmarkId = newLandmarkId
                        latchedYaw = newYaw
                        Log.d("ARCoreArrow", "Latched new landmark=${newLandmarkId}, yaw=${"%.1f".format(latchedYaw)}")
                    } else {
                        // Keep previous yaw until a NEW match occurs
                    }
                }
        ...
        update = { arSceneView ->
            ...
            // Update arrow rotation if direction changes
            val controller = (arSceneView.tag as? ArrowController) ?: ArrowController(arSceneView.context).also { arSceneView.tag = it }
            controller.updateYaw(directionDeg)
        },
        onRelease = { view ->
    ...
    try {
        ...
        try {
            ...
            if (gray != null) {
                ...
                try {
                    if (safeClone != null) {
                        vm.processGrayMatForFeatureMatching(safeClone)
                    }
                } finally {
                ...
                if (bitmap != null) {
                    try {
                        vm.processFrameForFeatureMatching(bitmap)
                    } catch (_: Exception) {
                    }
                }
            }
